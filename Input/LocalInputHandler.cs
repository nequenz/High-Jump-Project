...
public abstract class LocalInputHandler : MonoBehaviour, IEnumerable<ActionBind>
{
    ...
    protected void AttachAction(InputActionIDs actionID, Action action, KeyMode mode, KeyCode key) => AttachAction((int)actionID,action,mode,key);
    ...
    public void SetActionEnableMode(InputActionIDs id, bool isEnabled) => SetActionEnableMode((int)id, isEnabled);

    public void EnableAction(InputActionIDs id) => SetActionEnableMode(id,true);

    public void DisableAction(InputActionIDs id) => SetActionEnableMode(id,false);
